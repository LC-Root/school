...
public interface RolesRepository extends JpaRepository<Roles, Integer> {
	...
	@Query("from Roles where name = ?1")
	List<Roles> queryByNameUseHQL(String name);
	
	//个人推荐使用一下方式i写sql语句，因为在使用上边的方式的话，jpa会先将不标准的sql转化为标准的sql语句执行，相对内部操作过程会多一步骤
	@Query(value="select * from t_roles where name = ?", nativeQuery=true)
	List<Roles> queryByNameUseSQL(String name);
